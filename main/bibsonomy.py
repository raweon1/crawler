...
class BibSonomy:
    ...
    def getPostsForUser(self, resource_type, user_name, tags=None, start=0, end=_MAX):
        ...
        return self._get_all_posts(
            partial(self.rest.getPostsForUser, resource_type=resource_type, user_name=user_name, tags=tags), start, end)

    def getPostsForTag(self, resource_type, tags, start=0, end=_MAX):
        return self._get_all_posts(partial(self.rest.getPostsForTag, resource_type=resource_type, tags=tags), start,
                                   end)
...
class JSON:
    ...
    def _decode_post(self, js):
        ...
        if "bookmark" in js:
            ...
            if "bibtex" in js:
                # added by Andre Hoennscheidt for year = empty
                try:
                    js["bibtex"]["year"]
                except KeyError:
                    js["bibtex"]["year"] = "NAN"
...
class REST:
    ...
    def __init__(self, user_name, api_key, data_format="json", protocol="https", hostname="www.bibsonomy.org",
                 path="/api"):
        """
    ...
    def getPostsForUser(self, resource_type, user_name, tags, start=0, end=_MAX_POSTS_PER_REQUEST):
        ...
        # get the data
        json = self._get("/users/" + quote(user_name, '') + "/posts?resourcetype=" + quote(
            self._get_resource_type(resource_type)) + tag_query + "&start=" + str(start) + "&end=" + str(end))
    ...
    def getPostsForTag(self, resource_type, tags, start=0, end=_MAX_POSTS_PER_REQUEST):
        ...
        # get the data
        json = self._get(
            "/posts?resourcetype=" + quote(self._get_resource_type(resource_type)) + tag_query + "&start=" + str(
                start) + "&end=" + str(end))
        return json
